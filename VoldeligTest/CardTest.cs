...
public class CardTest
{
    ...
    [Fact]
    public async Task DeleteEmpTest()
    {
        var client = new Voldelig(_configuration);
        var employee = new Employees() { EmployeeNumber = "101028" };

        var employeeUpdated = await client.Authenticate().Card(ActionType.Delete, employee);
    ...
    [Fact]
    public async Task CreateEmpTest()
    {
        var client = new Voldelig(_configuration);
        var employee = new Employees() { Name1 = "VOLDELIG", Name2 = "Østerbrogade 323", Country = CountryType.DENMARK, Gender = GenderType.MALE, CreatedDate = DateTime.MinValue };

        var employeeUpdated = await client.Authenticate().Card(ActionType.Create, employee);
